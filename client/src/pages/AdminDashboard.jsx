    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-amber-100 pb-6">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="bg-amber-100/70 p-1 rounded-2xl flex items-center border border-amber-200">
            <button
              onClick={() => setActiveTab('destinations')}
              className={`px-4 py-2 rounded-xl text-xs font-bold transition flex items-center gap-1.5 cursor-pointer ${
                activeTab === 'destinations'
                  ? 'bg-[#0A192F] text-amber-300 shadow-sm'
      ...
      <div className="p-4 rounded-2xl bg-amber-500/10 border border-amber-300/80 text-amber-950 text-xs font-medium flex items-center gap-3">
        <ShieldAlert className="w-5 h-5 text-amber-700 shrink-0" />
        <div>
          <span className="font-bold">Strict Ownership Deletion Rule Active:</span> Aap Admin Dashboard me wahi record ya Admin account delete kar sakte hain jo aapne add/create kiya tha. Dusre Admin ke add kiye admin accounts deletion permissions locked zone me rahenge.
        </div>
      </div>
      ...
      {activeTab === 'admins' && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="bg-white p-5 rounded-2xl border border-amber-900/10 shadow-xs">
              <div className="flex items-center justify-between text-slate-400 mb-2">
                <span className="text-xs font-bold uppercase text-amber-800/70">Total Admin Accounts</span>
                <Users className="w-4 h-4 text-amber-600" />
              </div>
              <span className="text-2xl font-black text-[#0A192F]">{admins.length}</span>
              <span className="text-[11px] text-slate-500 block mt-0.5">Active Admin Privileges</span>
            </div>

            <div className="bg-white p-5 rounded-2xl border border-amber-900/10 shadow-xs">
              <div className="flex items-center justify-between text-slate-400 mb-2">
                <span className="text-xs font-bold uppercase text-amber-800/70">Created By You</span>
              ...
              <span className="text-[11px] text-slate-500 block mt-0.5">Deletable by your account</span>
            </div>

            <div className="bg-white p-5 rounded-2xl border border-amber-900/10 shadow-xs">
              <div className="flex items-center justify-between text-slate-400 mb-2">
                <span className="text-xs font-bold uppercase text-amber-800/70">Deletion Policy</span>
                <Lock className="w-4 h-4 text-amber-600" />
              </div>
              <span className="text-lg font-black text-[#0A192F]">Creator-Only</span>
              <span className="text-[11px] text-amber-800 font-semibold block mt-0.5">Strict Permission Guard</span>
            </div>
          </div>

          {/* Add New Admin Form Panel */}
          {isAddingAdmin && (
            <form onSubmit={handleAddAdminSubmit} className="bg-white rounded-3xl p-6 sm:p-8 border border-amber-200 shadow-xl space-y-6 animate-fade-in">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-bold text-amber-800/70 uppercase block mb-1">Admin Full Name</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="text-xs font-bold text-amber-800/70 uppercase block mb-1">Admin Email Address</label>
                  <input
                    type="email"
                    required
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-bold text-amber-800/70 uppercase block mb-1">Password</label>
                  <input
                    type="password"
                    required
                ...
                <div>
                  <label className="text-xs font-bold text-amber-800/70 uppercase block mb-1">Department / Role</label>
                  <input
                    type="text"
                    required
              ...
              <div className="flex justify-end gap-3 pt-2">
                <button
                  ...
                  className="px-5 py-2.5 rounded-xl bg-amber-50 text-amber-950 text-xs font-bold cursor-pointer"
                >
                  Cancel
                </button>
                <button
          ...
          <div className="bg-white rounded-3xl p-6 sm:p-8 border border-amber-900/10 shadow-xs space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-bold text-[#0A192F]">Active Admin Accounts ({admins.length})</h3>
              ...
              <table className="w-full text-left text-xs text-slate-600">
                <thead className="bg-amber-50 text-[#0A192F] font-bold border-b border-amber-200">
                  <tr>
                    ...
                    <th className="p-3">Added By (Creator Admin)</th>
                    <th className="p-3 text-right">Action (Delete)</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-amber-100/60">
                  {admins.map((adm) => {
                    ...
                    return (
                      <tr key={admId} className="hover:bg-amber-50/40">
                        <td className="p-3 font-bold text-[#0A192F] flex items-center gap-2.5">
                          <img
                            src={adm.avatar || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=200&q=80'}
                            alt={adm.name}
                            className="w-8 h-8 rounded-full object-cover border border-amber-300"
                          />
                          <div>
                            <span className="block font-bold text-xs text-[#0A192F]">{adm.name}</span>
                            <span className="text-[10px] text-amber-800 font-mono font-bold uppercase">{adm.role || 'Admin'}</span>
                          </div>
                        </td>

                        <td className="p-3 font-mono font-semibold text-slate-700">{adm.email}</td>
                        ...
                        <td className="p-3">
                          {isCreator ? (
                            <span className="px-2.5 py-1 rounded-full bg-emerald-100 text-emerald-800 font-bold text-[10px] border border-emerald-300 flex items-center gap-1 w-fit">
                              <UserCheck className="w-3 h-3" />
                              Added by You
                            </span>
                          ) : (
                            <span className="px-2.5 py-1 rounded-full bg-slate-100 text-slate-600 font-semibold text-[10px] border border-slate-300 block w-fit">
                        ...
                        <td className="p-3 text-right">
                          {isCreator ? (
                            <button
                              ...
                              className="px-3 py-1.5 rounded-xl bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-300 font-bold transition flex items-center gap-1.5 ml-auto cursor-pointer"
                            ...
                            <button
                              onClick={() => handleDeleteAdmin(adm)}
                              className="px-3 py-1.5 rounded-xl bg-slate-100 text-slate-400 border border-slate-200 font-medium cursor-not-allowed opacity-60 flex items-center gap-1.5 ml-auto"
